...
public class Questionnaire : IHttpHandler, IRequiresSessionState
{
    ...
    public void GetQuestionnaireResultsDownload()
    {
        ...
        if (_typeID == 1)
        {
            dt = APQuestionnaireManager.GetQuestionnaireResults(projectID, _qid, _clientID, fromDate, toDate, _status, 1, totalCount);
        }
        else if (_typeID == 2)
        {
    ...
    public void GetQuestionnaireResults() 
    {
        ...
        if (_typeID == 1)
        {
            dt = APQuestionnaireManager.GetQuestionnaireResults(projectID, _qid, _clientID, fromDate, toDate, _status, pageFrom, pageTo);
        }
        else if (_typeID == 2)
        {
