...
public class QuestionAnswer : IHttpHandler, IRequiresSessionState
{
        ...
        try
        {
            ...
            switch (requestType)
            {
                ...
                case 9:
                    ...
                    break;
                case 10:
                    CalculateQuestionScoreWithoutSave();
                    break;
                case 11:
                    GetQuestionAnswer();
                    break;
                case 12:
                    GetClientAppealQuestionsDownload();
                    break;
                case 13:
                    GetWrongQuestions();
                    break;
    ...
    public void GetResultAnswers()
    {
        int resultID = HttpContext.Current.Request.QueryString["resultID"].ToInt(0);

        string jsonResult = "[";
        DataTable dtAnswers = APQuestionnaireManager.GetAnswersForResult(resultID);
        if (dtAnswers != null && dtAnswers.Rows.Count > 0)
        {
            foreach (DataRow row in dtAnswers.Rows)
            {
                int answerID = row["ID"].ToString().ToInt(0);
                int questionID = row["QuestionID"].ToString().ToInt(0);
                string totalScore = row["TotalScore"].ToString();
                string description = row["Description"].ToString();
                description = CommonFunction.StringToJson(description);
                DataTable dt = APQuestionManager.GetAnswerOptions(answerID);

                string answerJson = "{ \"ID\": " + answerID + ", \"QuestionID\": " + questionID + ", \"TotalScore\": " + totalScore + ",\"Description\": \"" + description + "\"}";
                string answerOptionsJson = JSONHelper.DataTableToJSON(dt);

                jsonResult += ("{\"Answer\":" + answerJson + ", \"AnswerOptions\": " + answerOptionsJson + "},");
            }
    ...
    public void GetQuestionAnswer() 
    {
        ...
        string answerOptionsJson = string.Empty;
        
        APAnswersDO answerDO = APQuestionManager.GetAnwserDO(_resultID, _questionID);
        if (answerDO != null && answerDO.ID > 0) 
        {
            int answerID = answerDO.ID;
            DataTable dt = APQuestionManager.GetAnswerOptions(answerID);
            answerJson = JSONHelper.ObjectToJSON(answerDO);
            answerOptionsJson = JSONHelper.DataTableToJSON(dt);
        }
        string jsonResult = "{\"Answer\":" + answerJson + ", \"AnswerOptions\": " + answerOptionsJson + "}";
        HttpContext.Current.Response.ContentType = "application/json";
        HttpContext.Current.Response.Write(jsonResult);
    }
    ...
    public void UploaAppealReply()
    {
        ...
        try
        {
            ...
            {
                string clientCode = row["网点编号"].ToString().Trim();
                string questionCode = row["题号"].ToString().Trim();
                ...
                {
                    string auditText = row["三方反馈"].ToString().Trim();
                    if (string.IsNullOrEmpty(auditText) == false)
                    {
                        APQuestionManager.DeleteQuestionAppealAuditNotes(resultID, questionID, (int)Enums.QuestionnaireAuditType.客户审核, WebSiteContext.Current.CurrentProject.QCLeaderUserID);
                    ...
                    string resultText = row["三方意见"].ToString().Trim();
                    int appealStatus = 0;
                    if (resultText == "还分")
                    {
                ...
                if (bNeedUpdateClientText == "是")
                {
                    ...
                    if (string.IsNullOrEmpty(clientText) == false)
                    {
                        ...
                        APQuestionManager.DeleteQuestionAppealAuditNotes(resultID, questionID, (int)Enums.QuestionnaireAuditType.客户审核, topClientUserDO.ID);
    ...
    public void UpdateQuestionnaireStatus()
    {
        ...
        DataTable dt = APQuestionnaireManager.GetQuestionAppeals(_qid, fromDate, toDate, _stageStatus, _statusID, _levelID, keyword, WebSiteContext.Current.CurrentUser.ClientID);
        int finished = 0;
        int leaveClient = 0;
    ...
    public void GetWrongQuestions()
    {
        ...
        int _statusID = 0;
        int.TryParse(statusID, out _statusID);
        DataTable dt = APQuestionnaireManager.GetWrongQuestions(_qid, fromDate, toDate, _stageStatus, _statusID, keyword, WebSiteContext.Current.CurrentUser.ClientID);
        string result = JSONHelper.DataTableToJSON(dt);
        HttpContext.Current.Response.ContentType = "application/json";
        HttpContext.Current.Response.Write(result);
    }

    public void GetClientAppealQuestionsDownload()
    {
        string qid = HttpContext.Current.Request.QueryString["qid"];
        string period = HttpContext.Current.Request.QueryString["period"];
        string stageStatus = HttpContext.Current.Request.QueryString["stageStatus"];
        ...
        dtOutput.Columns.Add("支行编号", typeof(string));
        dtOutput.Columns.Add("支行名称", typeof(string));
        ...
        dtOutput.Columns.Add("扣分原因", typeof(string));
        dtOutput.Columns.Add("支行申诉", typeof(string));
        dtOutput.Columns.Add("分行意见", typeof(string));
        dtOutput.Columns.Add("总行意见", typeof(string));
        if (dt != null && dt.Rows.Count > 0)
        {
            foreach (DataRow row in dt.Rows)
            {
                ...
                newRow["支行编号"] = row["Code"].ToString();
                newRow["支行名称"] = row["Name"].ToString();
                ...
                newRow["得分"] = row["Score"].ToString();
                newRow["扣分原因"] = row["OptionText"].ToString();
                newRow["支行申诉"] = row["AuditNotes"].ToString();
                newRow["分行意见"] = row["AuditNotes2"].ToString();
                newRow["总行意见"] = row["AuditNotes4"].ToString();
                if (string.IsNullOrEmpty(row["AuditNotes2"].ToString()))
                {
                    newRow["分行意见"] = "未填写内容";
                }
                if (string.IsNullOrEmpty(row["AuditNotes4"].ToString()))
                {
                    newRow["总行意见"] = "未填写内容";
                }
        ...
        {
            if (dc.ColumnName != "标准分" && dc.ColumnName != "得分")
            {
    ...
    public void GetFileNumberByTempCode() 
    {
        ...
        int fileNumberImage = APQuestionManager.GetFileNumberByTempCodeFileType(tempCode, (int)Enums.FileType.图片);
        int fileNumberAudio = APQuestionManager.GetFileNumberByTempCodeFileType(tempCode, (int)Enums.FileType.录音);
        int fileNumberVideo = APQuestionManager.GetFileNumberByTempCodeFileType(tempCode, (int)Enums.FileType.录像);
        string jsonResult = fileNumber.ToString() + "-" + fileNumberImage.ToString() + "-" + fileNumberAudio.ToString() + "-" + fileNumberVideo.ToString();
        HttpContext.Current.Response.ContentType = "text/plain";
        HttpContext.Current.Response.Write(jsonResult);
    }
    ...
    public void CalculateQuestionScoreWithoutSave()
    {
        string questionID = HttpContext.Current.Request.QueryString["questionID"];
        string selectOptions = HttpContext.Current.Request.QueryString["selectOptions"];
