<%@ WebHandler Language="C#" Class="Upload_ICTR" %>
...
public class Upload_ICTR : IHttpHandler, IRequiresSessionState
{
        ...
        try
        {
            ...
            switch (requestType)
            {
                ...
                case 99:
                    ...
                    break;
                case 7:
                    UploadKuaiResultDataByUrl();
                    break;
                case 8:
                    UploadXinZengResultDataByUrl();
                    break;
                default:
                    break;
            }
    ...
    public void UploadXinZengResultDataByUrl()
    {
        ...
        try
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                for (int i = 1; i < dt.Rows.Count; i++)
                {
                    ...
                    string photoAddress = row["09"].ToString();//拍照地址
                    string photoTime = row["10"].ToString();//拍照时间
                    string watermark = photoAddress + " " + photoTime;
                    LocateInfo resultLocation = new LocateInfo();
                    if (string.IsNullOrEmpty(photoAddress) && string.IsNullOrEmpty(photoTime))
                    {
                        watermark = GetWatermarkByID(id, row["开始填答时间"].ToString(), dtGPS);
                        resultLocation = GetLngLatByID(id, dtGPS);
                    }
                    ...
                    int clientID = 0;
                    string clientCode = row["NO"].ToString();//门店的DP编号
                    string shopName = row["A1"].ToString();//门店名称
                    string address =  row["A2"].ToString();
                    string cityName = row["na5"].ToString();//城市名称

                    string q03 = row["na3"].ToString();
                    string q07 = row["na7"].ToString();
                    string q08 = row["na10"].ToString();

                    row["03"] = q03;
                    row["07"] = q07;
                    row["08"] = q08;

                    string locationX = row["na24"].ToString();
                    string locationY = row["na25"].ToString();
                    string parentCompanyName = row["na26"].ToString();
                    string parentCityCode = row["na13"].ToString();

                    string companyName = row["02"].ToString();
                    LocateInfo clientLocation = new LocateInfo();
                    if (!string.IsNullOrEmpty(locationX) && !string.IsNullOrEmpty(locationY))
                    {
                        clientLocation = MapHelper.gcj02_To_Bd09(locationY.ToDouble(0), locationX.ToDouble(0));
                        locationX = clientLocation.Longitude.ToString();
                        locationY = clientLocation.Latitude.ToString();
                    }
                    ...
                    if (clientDO == null || clientDO.ID < 0)
                    {
                        ...
                        APClientsDO parentClientDO = APClientManager.GetClientByPostcode(parentCityCode, projectID);
                        if (parentClientDO != null && parentClientDO.ID > 0)
                        {
                            parentID = parentClientDO.ID;
                            provinceCode = parentClientDO.Province;
                            cityCode = parentClientDO.City;
                            districtCode = parentClientDO.District;
                        }
                        ...
                        clientDO.Status = 0;
                        clientID = BusinessLogicBase.Default.Insert(clientDO);
                        clientDO.ID = clientID;
                    }
                    else
                    {
                        clientID = clientDO.ID;
                        clientLocation.Longitude = clientDO.LocationCodeX.ToDouble(0);
                        clientLocation.Latitude = clientDO.LocationCodeY.ToDouble(0);
                    }

                    APQuestionnaireDeliveryDO deliveryDO = APQuestionnaireManager.GetQuestionnaireDeliveryDO(questionnaireID, clientID, fromDate, toDate, (int)Enums.UserType.访问员);
                    if (deliveryDO == null || deliveryDO.ID <= 0)
                    {
                        deliveryDO = new APQuestionnaireDeliveryDO();
                        deliveryDO.AcceptUserID = 6;
                        deliveryDO.ClientID = clientID;
                    ...
                    if (resultLocation != null && resultLocation.Latitude > 0 && resultLocation.Longitude > 0)
                    {
                        double distance = MapHelper.GetDistance(clientLocation.Latitude, clientLocation.Longitude, resultLocation.Latitude, resultLocation.Longitude);
                        resultDO.LocationX = resultLocation.Longitude;
                        resultDO.LocationY = resultLocation.Latitude;
                    ...
                    try
                    {
                        int questionID = APQuestionManager.GetQuestionIDByCode2(questionnaireID, "A06");
                    ...
                    for (int j = 0; j < dtQuestions.Rows.Count; j++)
                    {
                        ...
                        else if (questionType == (int)Enums.QuestionType.上传题)
                        {
                            ...
                            string formatFileName = parentCompanyName + "-" + cityName + "-" + shopName + "-" + questionName + extestion;
                            ...
                            SaveResultImageFile(tempFilePath, file_physicalPath, file_vituralPath, projectID, resultID, questionID, answerID,
                                fromDate.ToString("yyyy-MM-dd"), clientCode, formatFileName, (int)Enums.FileType.图片, watermark, (int)Enums.DocumentType.执行上传);
                    ...
                    string g3Answer = row["G3"].ToString();//是否发放物料
                    if (string.IsNullOrEmpty(g3Answer))
                    {
                        int questionID = APQuestionManager.GetQuestionIDByCode2(questionnaireID, "G3");
                        int answerID = 0;
                        APAnswersDO answerDO = APQuestionManager.GetAnwserDO(resultID, questionID);
                        if (answerDO == null || answerDO.ID <= 0)
                        {
                        ...
                        APOptionsDO option = APQuestionManager.GetOptionDO(questionID, "非地方站城市");
                        if (option != null && option.ID > 0)
                        {
                            APAnswerOptionsDO optionDO = APQuestionManager.GetAnswerOption(answerID, option.ID);
                            if (optionDO == null || optionDO.ID <= 0)
    ...
    public void UploadKuaiResultDataByUrl()
    {
        byte[] byts = new byte[HttpContext.Current.Request.InputStream.Length];
        HttpContext.Current.Request.InputStream.Read(byts, 0, byts.Length);
        string jsonData = System.Text.Encoding.Default.GetString(byts);
        ...
        try
        {
            ...
            if (dt.Columns.Contains("A06_1") == false)
            {
                dt.Columns.Add("A06_1");
            }
            if (dt.Columns.Contains("A6") == true)
            {
                dt.Columns["A6"].ColumnName = "A06";
            }
            if (dt.Columns.Contains("A7") == true)
            {
                dt.Columns["A7"].ColumnName = "A07";
            }
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                for (int i = 1; i < dt.Rows.Count; i++)
                {
                    ...
                    string brandName = row["na7"].ToString();//品牌名称
                    string cityName = row["05"].ToString();//城市名称
                    string companyName = row["na10"].ToString();//分公司名称
                    string address = row["A07"].ToString();//门店地址
                    row["A06_1"] = shopName;
                    ...
                    if (clientDO == null || clientDO.ID < 0)
                    {
                        ...
                        clientDO.Name = brandName + shopName;
                    ...
                    for (int j = 0; j < dtQuestions.Rows.Count; j++)
                    {
                        ...
                        else if (questionType == (int)Enums.QuestionType.上传题)
                        {
                            ...
                            string extestion = Path.GetExtension(tempFilePath);
                            //分公司-城市-品牌名称-商户名称-指标名称.jpg
                            string formatFileName = companyName + "-" + cityName + "-" + brandName + "-" + shopName + "-" + questionName + extestion;
                            formatFileName = CommonFunction.FixedSpecialCharsInFileName(formatFileName);
                            SaveResultImageFile(tempFilePath, file_physicalPath, file_vituralPath, projectID, resultID, questionID, answerID,
                        ...
                        else if (questionType == (int)Enums.QuestionType.单选题)
                        {
                            ...
                            if (option != null && option.ID > 0)
                            {
                                ...
                                optionDO.AnswerID = answerID;
                                optionDO.OptionID = option.ID;
                                if (option.BAllowText) {
                                    string questionCode_open = questionCode + "_open";
                                    if (dt.Columns.Contains(questionCode_open))
                                    {
                                        string openTextValue = row[questionCode_open].ToString();
                                        optionDO.OptionText = value;
                                    }
                                }
        ...
        try
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                for (int i = 1; i < dt.Rows.Count; i++)
                {
                    ...
                    string sampleType = row["LX"].ToString();
                    string b1 = row["B1"].ToString();
                    string cityName = row["05"].ToString();
                    if (sampleType != "正式样本" && cityName != "南阳市")
                    {
                        continue;
                    }
                    if (b1 == "不支持")
