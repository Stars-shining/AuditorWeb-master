    ...
    public void GetUserMenusByCurrentUser()
    {
        ...
        if (_bIsInProject)
        {
            ...
            if (WebSiteContext.Current.CurrentUser.ClientID > 0) 
            {
                ...
                int maxLevelID = APClientManager.GetClientMaxLevel(WebSiteContext.CurrentProjectID);
                if (userType == (int)Enums.UserType.客户 && clientDO.LevelID != maxLevelID)
                {
                    //非支行账号
                    menuPage = "CommonProjectLeftClient1.htm";
                }
            }
        }
