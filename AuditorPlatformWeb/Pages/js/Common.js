...
function ConvertJsonTimeToSeconds(jsonTime) {//json日期格式转换为正常格式
    try {
        return jsonTime.Hours * 3600 + jsonTime.Minutes * 60 + jsonTime.Seconds;
    } catch (ex) {
        return "";
    }
}
function ConvertJsonTimeToString(jsonTime) {//json日期格式转换为正常格式
    try {
        var hours = jsonTime.Hours < 10 ? "0" + jsonTime.Hours : jsonTime.Hours;
        var minutes = jsonTime.Minutes < 10 ? "0" + jsonTime.Minutes : jsonTime.Minutes;
        var seconds = jsonTime.Seconds < 10 ? "0" + jsonTime.Seconds : jsonTime.Seconds;
...
//选项叠加扣分 = 6,
//等于选项分 =7
function CalculateQuestionScore(countType, fullScore, selectedOptions, correctOptions) {
    var score = 0;
    //不得分不扣分 = 5,
    if (countType == 5) {
        return score;
    ...
    var success = false;
    var matchedNumber = 0;
    if (correctOptions != null && correctOptions.length > 0 && correctOptions.length == selectedOptions.length) {
        $.each(correctOptions, function (i, item) {
            var optionID = item;
            $.each(selectedOptions, function (j, optionDO) {
                if (optionDO.ID == optionID) {
                    matchedNumber++;
                    return false;
                }
            });
    ...
    //错误扣分 = 2,
        ...
        if (success == false) {
            score = -Math.abs(fullScore);
        }
    }
    //选项分总和 = 4,
    else if (countType == 4) {
        var totalScore = fullScore;
        $.each(selectedOptions, function (i, optionDO) {
    ...
    //等于选项分 =7
        ...
        if (selectedOptions.length > 0) {
            var optionDO = selectedOptions[0];
            score = optionDO.Score;
        }
    }
    return score;
